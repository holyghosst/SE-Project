...
public class MainClient {
	...
	NetworkConverter networkConverter = new NetworkConverter(serverBaseUrl);
	networkConverter.registerPlayer("Oleksandr", "Lymarenko", "lymarenkoo02", gameId);

	System.out.println("Registered with ID:" + networkConverter.getPlayerID());

	while (!networkConverter.isMyTurn())
	    ;
	System.out.println("Both clients registered");

	HalfMapGenerator mapGenerator = new HalfMapGenerator();
	HalfMap map = mapGenerator.generateHalfMap();
	map.printOut();
	networkConverter.sendHalfMap(map);

	while (!networkConverter.isMyTurn())
	    ;
	System.out.println("Both clients sent the map!");
